...
class PharmacyApp:
    ...
    def update_image(self, index):
        ...
        self.root.after(50, self.update_image, (index + 1) % len(self.frames))
    ...
    def create_gui(self):
        ...
        self.notebook = ttk.Notebook(self.root, style='My.TNotebook',width=800)
        ...
        self.sell_button = tk.Button(self.medicine_tab, font=20, text="Sell medicine", background="cadetblue", foreground="white", command=self.open_sell_window)
    ...
    def on_tab_selected(self, event):
        ...
        if current_tab == "Patients":
            ...
            self.patient_insert_button.place(x=200, y=200)
        else:
    ...
    def delete_selected_patient(self):
        ...
        if selected_item:
            confirm = messagebox.askyesno("Confirm Deletion", "Are you sure you want to delete the selected patient?")
            if confirm:
                patient_id = self.patients_tree.item(selected_item, "text")
                self.cur.execute("DELETE FROM Patient WHERE Patient_id=?", (patient_id,))
                self.conn.commit()
                self.refresh_patients_table()
    ...
    def create_doctors_gui(self):
        ...
        self.employee_delete_button = tk.Button(self.employees_tab, font=20, text="Delete Employee", background="cadetblue", foreground="white", command=self.delete_selected_employee)
        self.employee_delete_button.place(x=350, y=150)

    def delete_selected_employee(self):
        selected_item = self.employees_tree.selection()
        if selected_item:
            confirm = messagebox.askyesno("Confirm Deletion", "Are you sure you want to delete the selected employee?")
            if confirm:
                employee_id = self.employees_tree.item(selected_item, "text")
                self.cur.execute("DELETE FROM Employee WHERE Emp_id=?", (employee_id,))
                self.conn.commit()
                self.refresh_employees_table()
    ...
    def create_patients_gui(self):
        ...
        self.refresh_patients_table()
        self.patient_delete_button = tk.Button(self.patients_tab, font=20, text="Delete Patient", background="cadetblue", foreground="white", command=self.delete_selected_patient)
        self.patient_delete_button.place(x=350, y=176)
